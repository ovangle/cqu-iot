...
from typing import AsyncIterable, AsyncIterator, TypeVar, cast
from asyncio import Future
import asyncio
import dataclasses
import json
from typing import Callable
from mech_arm_mqtt.schema.session import MechArm
from paho.mqtt.client import MQTTv311, CallbackAPIVersion
from asyncio_mqtt import Client as MQTTClient, ProtocolVersion


from .env import RemoteClientSettings


class MyCobotRemoteClient(MQTTClient, MechArm):
    def __init__(self, settings: RemoteClientSettings):
        super().__init__(
            hostname=settings.MQTT_HOST,
            port=settings.MQTT_PORT,
            client_id=settings.REMOTE_CLIENT_ID,
            protocol=ProtocolVersion.V311,
            username=settings.MQTT_USER,
            password=settings.MQTT_PASSWORD,
        )
        self.client_id = settings.REMOTE_CLIENT_ID
        self.application_id = settings.MECHARM_APP_ID

    async def events(self) -> AsyncIterator[MechArmEvent]:
        await self.subscribe(self.mecharm_broadcast_topic)
    ...
    async def _dispatch_action(
        ...
        await self.publish(topic, json.dumps(action_to_json_object(action)))
    ...
    async def begin_session(
        ...
        if isinstance(begin_session_response, SessionCreated):
            return MyCobotRemoteSession(
                self, self.application_id, begin_session_response.session.session_id
            )
        else:
